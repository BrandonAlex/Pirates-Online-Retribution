...
_modifier_names = dict([(name, number)
                         for number in xrange(len(_modifiers))
...
# An int in xrange(1 << len(_modifiers)) represents a combination of modifiers
...
for s in _states:
    r = 0
    for i in xrange(len(_modifiers)):
        if (1 << i) & s:
            r |= _modifier_masks[i]
...
_type_names = dict([(name, number)
                     for number in xrange(len(_types))
