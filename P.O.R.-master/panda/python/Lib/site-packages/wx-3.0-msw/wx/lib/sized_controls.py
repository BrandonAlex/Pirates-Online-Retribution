...
class TableSizer(wx.PySizer):
    ...
    def CalcMin(self):
        ...
        self.row_widths = [0 for x in xrange(0, numrows)]
        self.col_heights = [0 for x in xrange(0, numcols)]
        currentRow = 0
        currentCol = 0
        counter = 0
        self.hgrow = 0
    ...
    def RecalcSizes(self):
        ...
        row_widths = [0 for x in xrange(0, numrows)]
        col_heights = [0 for x in xrange(0, numcols)]
        item_sizes = [0 for x in xrange(0, len(self.GetChildren()))]
        grow_sizes = [0 for x in xrange(0, len(self.GetChildren()))]
        ...
        for item in self.GetChildren():
            ...
            if item.IsShown():
                rowstart = itempos[0] 
                for row in xrange(0, currentRow):
                    rowstart += row_widths[row]
                    
                colstart = itempos[1]
                for col in xrange(0, currentCol):
