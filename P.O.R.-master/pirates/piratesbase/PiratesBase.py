...
class PiratesBase(OTPBase):
    ...
    def __init__(self):
        ...
        # Cache
        if self.config.GetBool('want-cache', True):
            print 'PiratesBase: want-cache #t'
            self.bamCache.setActive(True)
            self.bamCache.setCacheModels(True)
            self.bamCache.setCacheTextures(True)
            self.bamCache.setCacheCompressedTextures(True)
        else:
            print 'PiratesBase: want-cache #f'
            self.bamCache.setActive(False)
            self.bamCache.setCacheModels(False)
            self.bamCache.setCacheTextures(False)
            self.bamCache.setCacheCompressedTextures(False)
    ...
    def startShow(self, cr):
        ...
        for name in gameServer.split(';'):
            url = URLSpec(name, 1)
            enableSsl = base.config.GetBool('enable-ssl', 0)
            if enableSsl:
                url.setScheme('s')
            else:
                url.setScheme('')
