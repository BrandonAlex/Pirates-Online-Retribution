...
##            dbg(indent=0)
            ...
            empty = True
            for pos in xrange(len(self._template)):
...
####            dbg('self._groupdigits:', self._groupdigits, 'self._formatcodes:', self._formatcodes)
            if self._groupdigits:
                new = ''
                cnt = 1
                for i in xrange(len(intStr)-1, -1, -1):
                    new = intStr[i] + new
                    if (cnt) % 3 == 0:
...
##        dbg('new mask: "%s"' % s, indent=0)
    ...
    def _calcFieldExtents(self):
        ...
        if self._mask:
            ...
            self.maskdict = {}
            for charnum in xrange( len( self._mask)):
...
##                dbg('decimal pos =', self._decimalpos)
                ...
                for i in xrange(self._decimalpos+1):
                    self._lookupField[i] = 0

                for i in xrange(self._decimalpos+1, len(self._mask)+1):
                ...
                for i in xrange(len(self._mask)+1):
                    self._lookupField[i] = 0
            else:
...
##        for i in xrange(len(self._mask)):
...
##        dbg('default set?', default_set)
        ...
        for pos in xrange(len(self._mask)):
####            dbg('pos:', pos)
...
####            dbg('isinstance(self, wx.lib.masked.numctrl.NumCtrl): "%s"' % self.__class__)
            ...
            if isinstance(self, wx.lib.masked.numctrl.NumCtrl):
                ...
                if self._masklength > 10:
                    ...
                    sizing_text = ""
                    for cn in xrange(self._masklength):
                        if cn % 2:
                            sizing_text += "M"
                        else:
                            sizing_text += "I"
...
##            dbg('cut newfield: "%s"' % newfield)
            ...
            left = ""
            for i in xrange(start, end - len(newfield)):
                if field._padZero:
                    left += '0'
...
##            dbg('old value: "%s"\nnew value: "%s"' % (value, newvalue))
    ...
    def _adjustKey(self, pos, key):
        ...
        field = self._FindField(pos)
        if field._forceupper and key in xrange(97,123):
            key = ord( chr(key).upper())

        if field._forcelower and key in xrange(65,90):
...
####        dbg('text: "%s"' % text)
        i = 0
        if len(text.rstrip()):
            for i in xrange( min( self._masklength-1, len(text.rstrip())), -1, -1):
...
##        dbg("MaskedEditMixin::_goHome; getPosOnly:", getPosOnly, indent=1)
        text = self._GetValue()
        for i in xrange(self._masklength):
            if self._isMaskChar(i):
                break
        pos = max(i, 0)
##        dbg(indent=0)
...
##            dbg('returned ""', indent=0)
            ...
            plain = ""
            for idx in xrange( min(len(self._template), len(text)) ):
...
##        dbg("current sel_start, sel_to:", sel_start, sel_to)
        ...
        newvalue = list(value)
        for i in xrange(sel_start, sel_to):
...
##                    dbg("%7s value[%d:%d] (%s) prev[%d:%d] (%s)" % (op, i1, i2, value[i1:i2], j1, j2, prev[j1:j2]))
                ...
                # first change wins:
                for next_op in xrange(len(code_5tuples)-1, -1, -1):
...
##                    dbg('searching forward...')
                    for next_op in xrange(len(code_5tuples)):
                        op, i1, i2, j1, j2 = code_5tuples[next_op]
