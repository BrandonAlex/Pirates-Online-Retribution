    ...
    def onKey(self,evt):
        ...
        if self.parent.editor.mode == self.parent.editor.ANIM_MODE:
            ...
            if (objUID, propertyName) in self.parent.editor.animMgr.keyFramesInfo:
                for i in xrange(len(self.parent.editor.animMgr.keyFramesInfo[(objUID,propertyName)])):
                ...
                for i in xrange(0, len(sortKeyList)-1):
                    for j in xrange(i+1, len(sortKeyList)):
...
class ObjectPropUITime(wx.Panel):
    def __init__(self, parent, label, value):
        ...
        self.uiHour = wx.Choice(self.uiPane, -1, choices=[str(x) for x in xrange(1, 13)])
        self.uiMin = wx.Choice(self.uiPane, -1, choices=[str(x) for x in xrange(0, 60, 15)])
