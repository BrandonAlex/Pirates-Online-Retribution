...
class Shape(ShapeEvtHandler):
    ...
    def OnDrawBranches(self, dc, erase = False):
        if self._attachmentMode != ATTACHMENT_MODE_BRANCHING:
            return
        for i in xrange(self.GetNumberOfAttachments()):
...
class PolygonShape(Shape):
    ...
    def SetSize(self, new_width, new_height, recursive = True):
        ...
        for i in xrange(max(len(self._points), len(self._originalPoints))):
    ...
    def ResetControlPoints(self):
        for i in xrange(min(len(self._points), len(self._controlPoints))):
    ...
    def Rotate(self, x, y, theta):
        ...
        for i in xrange(len(self._originalPoints)):
