...
class ClientServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def doRPC(self, code, hash):
        sender = self.air.getMsgSender()

        h = hashlib.sha512(code * 5 + self.air.getApiKey() + code[::-5] * 14)
        h.update('rpcsalt')
        h.update(config.GetString('csm-secret', 'dev'))
        dg = h.digest()

        if len(dg) != len(hash):
            self.killConnection(sender, 'An operation is already underway: ?.')
            return
        ...
        if result:
            self.killConnection(sender, 'An operation is already underway: <unknown>.')
            return
        ...
        try:
            ...
            ret = describeException()

        self.air.systemMessage(ret, sender)
