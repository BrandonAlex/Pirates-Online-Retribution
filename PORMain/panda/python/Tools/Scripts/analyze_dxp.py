...
def merge_profile():
    ...
    with _profile_lock:
        ...
        if has_pairs(new_profile):
            for first_inst in xrange(len(_cumulative_profile)):
                for second_inst in xrange(len(_cumulative_profile[first_inst])):
                    _cumulative_profile[first_inst][second_inst] += (
                        new_profile[first_inst][second_inst])
        else:
            for inst in xrange(len(_cumulative_profile)):
