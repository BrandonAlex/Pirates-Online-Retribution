...
def processFunction(handle, function, isConstructor = False):
    for i_wrapper in xrange(interrogate_function_number_of_python_wrappers(function)):
        ...
        first = True
        for i_param in xrange(interrogate_wrapper_number_of_parameters(wrapper)):
...
def processType(handle, type):
    ...
    derivations = [ translated_type_name(interrogate_type_get_derivation(type, n)) for n in xrange(interrogate_type_number_of_derivations(type)) ]
    ...
    if interrogate_type_is_enum(type):
        ...
        for i_value in xrange(interrogate_type_number_of_enum_values(type)):
