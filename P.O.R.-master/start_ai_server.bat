...
set ASTRON_IP=127.0.0.1
set EVENT_LOGGER_IP=127.0.0.1:6668
set DISTRICT_NAME=POR-Dev
...
:main
%PYTHON_PATH% -m pirates.ai.ServiceStart --base-channel %BASE_CHANNEL% --max-channels %MAX_CHANNELS% --state-server %STATE_SERVER% --astron-ip %ASTRON_IP% --event-logger-ip %EVENT_LOGGER_IP% --district-name "%DISTRICT_NAME%"
pause
goto main
